...
import com.hackathon.beve.config.EmbeddedKafka;
import com.hackathon.beve.config.KafkaSseConsumer;
import com.hackathon.beve.config.KafkaSseProducer;
...
class BeveAppKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/beve-app-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/beve-app-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/beve-app-kafka/unregister"));
                return;
            }
        }
